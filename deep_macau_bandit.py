...
save_path_next = "/tmp/model-next.ckpt"
lrates    = [1e-3, 1e-4, 3e-5]
train_rmse_path = np.zeros( (len(lrates), 200) )

with tf.Session() as sess:
  sess.run(tf.initialize_all_variables())
  saver.save(sess, save_path_next)

  best_train_sse = np.inf

  for epoch in range(200):
    rIdx = np.random.permutation(Ytrain.shape[0])
    epoch_train_rmse = np.inf
    os.rename(save_path_next, save_path)
    ...
    for i in range(len(lrates)):
      ...
      test_sse = sess.run(y_loss,  feed_dict = feed_test)
      train_sse = sess.run(y_loss, feed_dict = feed_train)
      ...
      train_rmse_path[i, epoch] = train_rmse

      if epoch < 20 and i == 0:
        saver.save(sess, save_path_next)
      if 20 <= epoch and epoch < 60 and i == 1:
        saver.save(sess, save_path_next)
      if 60 <= epoch and i == 2:
        saver.save(sess, save_path_next)


path_df = pd.DataFrame(data = train_rmse_path.transpose(), columns=lrates)
path_df.to_csv("train_rmse_path.csv")
