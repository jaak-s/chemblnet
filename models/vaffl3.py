...
Zbias = tf.Variable(np.zeros(h1_size), dtype=tf.float32, name="Zbias")
Vbias = tf.Variable(np.zeros(h1_size), dtype=tf.float32, name="Vbias")
...
## means
Zmean_b = tf.add(tf.nn.embedding_lookup(Z.mean, x_idx_comp), Zbias)
#h1      = tf.nn.embedding_lookup_sparse(beta.mean, sp_ids, None, combiner = "sum") + Zmean_b
h1      = Zmean_b
h1_b    = tf.nn.embedding_lookup(h1, y_idx_comp)
Vmean_b = tf.add(tf.nn.embedding_lookup(V.mean, y_idx_prot), Vbias)
